...
- Do not use `insecure: true`, `ssl_verify`, or `--skip-ssl-verify` in `config.json` or the CLI - these are not honored
- For self-signed dev orchestrators, use the `--trust-all` global flag
- Keep CA certificates up to date
- Never bypass SSL verification in production

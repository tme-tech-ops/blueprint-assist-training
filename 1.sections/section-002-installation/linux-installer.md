...
PASS Binary size OK (134708192 bytes)

Package structure
-----------------
PASS install.sh present
PASS VERSION.txt present (Version: v0.31.0-20260914XXXXXX)
PASS knowledge/ directory present (18 plugin dirs)
PASS skills/ directory present (10 skill dirs)
...
> **v0.31.0 breaking change**: The `bpa` command alias is removed - only `dap-bpa` ships. Update any scripts or PATH aliases that reference `bpa` to use `dap-bpa` instead.
...
# or
dap-bpa setup-ide jetbrains
# or
dap-bpa setup-ide antigravity
# or
dap-bpa setup-ide cline
# or
dap-bpa setup-ide codex
```

This installs the dap-bpa skills and rules into the appropriate agent configuration directory:

- **Devin**: `.agents/skills/` + `.agents/rules/`
- **Claude Code**: `~/.claude/`
- **VS Code**: `~/.vscode/`
- **Cursor / Antigravity**: `.agents/`
- **JetBrains IDEs**: `.claude/`
- **Cline**: `.cline/`
- **Codex**: `.agents/skills/` + `.agents/rules/`

> **Note**: The `windsurf` IDE target is replaced by `devin` in v0.31.0. Existing Windsurf installs are still synced by `upgrade` and scanned by `verify-skills`.

## Extensibility Options

For enhanced agentic AI code assistance, you can install the Devin CLI on Linux (including WSL).
...
# Restart your shell or source your profile so the PATH update takes effect
source ~/.bashrc
...
After installing, configure Devin to work with dap-bpa for error-fixing assistance:

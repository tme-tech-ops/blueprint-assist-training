...
> **v0.31.0 breaking change**: The `bpa` command alias is removed - only `dap-bpa` ships. The `windsurf` IDE target is replaced by `devin`.
...
> **Note**: The `windsurf` IDE target is replaced by `devin` in v0.31.0.
...
2. Run: `dap-bpa setup`
3. Run: `dap-bpa setup-ide devin` (or cursor / claude-code / codex)

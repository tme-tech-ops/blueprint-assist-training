...
dap-bpa is **agent-agnostic**: it ships the skills and the `dap-bpa` command, and you bring whatever IDE/agent your team prefers. As of build v0.31.0, `dap-bpa setup-ide` installs the dap-bpa skills and rules into any of these targets: **Devin**, **Claude Code**, **Cursor**, **JetBrains IDEs** (Claude Code extension), **VS Code** (Claude Code extension), **Antigravity**, **Cline**, and **Codex**. Two of these targets cover more than one agent:

- **`antigravity`** also covers **Gemini CLI** and **Goose** - they load from the same directory.
- **Devin** and **Codex** install to `.agents/skills/` + `.agents/rules/`.

> **v0.31.0 breaking change**: The `windsurf` IDE target is replaced by `devin`. The `bpa` command alias is removed - only `dap-bpa` ships.

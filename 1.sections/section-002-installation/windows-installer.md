...
> **Note**: The NSIS installer is provided as a single packed executable file (`bpa-win-x64-v0.29.3-*-setup.exe`) that includes all necessary components (binary, knowledge base, skills, and rules) for a complete installation.
...
As of v0.29.3, the NSIS installer places the binary at `%USERPROFILE%\bin\bpa.exe`, stages the knowledge base, skill sources, and script library under `%USERPROFILE%\.blueprint-assist\`, and adds the binary directory to your PATH (open a new terminal for the PATH change to take effect).
...
# Example: bpa-win-x64-v0.29.3-*.zip

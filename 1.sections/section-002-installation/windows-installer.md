...
> **Note**: The NSIS installer is provided as a single packed executable file (`bpa-win-x64-v0.31.0-*-setup.exe`) that includes all necessary components (binary, knowledge base, skills, and rules) for a complete installation.
>
> **v0.31.0 breaking change**: The `bpa` command alias is removed - only `dap-bpa` ships. Update any scripts or PATH aliases that reference `bpa` to use `dap-bpa` instead. Install scripts detect and remove stale `bpa` binaries left by older installers.
...
As of v0.31.0, the NSIS installer places the binary at `%USERPROFILE%\bin\dap-bpa.exe`, stages the knowledge base, skill sources, and script library under `%USERPROFILE%\.blueprint-assist\`, and adds the binary directory to your PATH (open a new terminal for the PATH change to take effect).
...
# Example: bpa-win-x64-v0.31.0-*.zip
...
If `dap-bpa` is not recognized after installation:

```powershell
# Add to your path if needed - point this at the directory containing
# dap-bpa.exe (e.g. C:\Users\<username>\bin\)
...
1. Install the Devin CLI using the PowerShell one-liner:

   ```powershell
   irm https://static.devin.ai/cli/setup.ps1 | iex
   ```

   Alternatively, download Devin for Windows from [https://cli.devin.ai/docs#windows](https://cli.devin.ai/docs#windows) and run the installer.
2. Open a new PowerShell session so the updated PATH takes effect

...
Both servers use the same five credential headers:
...
| Tool | Description |
| --- | --- |
| `get_datetime` | Current server time |
| `list_node_types` | List node types for a plugin |
| `get_node_type` | Get node type schema |
| `get_plugin_docs` | Plugin documentation |
| `get_node_type_docs` | Node type documentation |
| `search_docs` | Search documentation |
| `find_blueprint_examples` | Find example blueprints |
| `get_blueprint_example` | Get a specific example |
| `lint_blueprint` | Validate blueprint YAML |
| `fetch_github_context` | Fetch context from GitHub |
...
All requests go to `POST /mcp`. The session lifecycle requires an `initialize` call first; subsequent calls reuse the session ID returned in the `Mcp-Session-Id` response header.
...
#### Call a knowledge tool (no credentials needed)
...
curl -s -X POST https://ai-bp-mcp-server.sp.dell.com/mcp \
  ...
  -d '{
    ...
    "params": {
      "name": "lint_blueprint",
      "arguments": {"blueprint_yaml": "tosca_definitions_version: ..."}
    }
  }'
```

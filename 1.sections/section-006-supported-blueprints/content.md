...
- **Plugin discovery**: reference blueprints show which plugins and node types solve a specific use case, how plugin imports are written, and how plugin-provided runtime attributes become capabilities
...
When evaluating a reference blueprint, pay attention to the plugin imports at the top of `blueprint.yaml` and the `type:` values under `node_templates`. Those two pieces tell you which plugin capabilities the blueprint depends on. If you are building a similar service, start by reusing the same plugin family and then inspect the plugin documentation before changing node properties or adding new nodes.

...
## Decision Trace: Understanding AI Reasoning (v0.30.1+)

When the agent analyzes a blueprint or proposes changes, it generates a **decision trace** - a structured record of the reasoning steps, tool calls, and conclusions that led to its output. Decision traces provide transparency into how the agent arrived at its recommendations, making it easier to verify, audit, and learn from its reasoning.
...
| `timestamp` | When the reasoning session occurred |
| `intent` | The user's original request or question |
| `steps` | Ordered list of reasoning steps with tool calls and intermediate conclusions |
| `tools_used` | Which CLI primitives and knowledge lookups were invoked |
| `final_decision` | The agent's conclusion or recommendation |
| `confidence` | How confident the agent is in its conclusion (when applicable) |
| `alternatives_considered` | Other options the agent evaluated and why they were rejected |

### Accessing Decision Traces

#### Via CLI

```bash
# List recent decision traces
dap-bpa decision-trace list

# Get a specific trace with full reasoning
dap-bpa decision-trace get <trace_id>

# Export traces to JSON for analysis or audit
dap-bpa decision-trace export --output traces.json
```

#### Via IDE

In your IDE, after the agent completes an analysis, you can ask:

- "Show me the decision trace for that analysis."
- "Why did you recommend that node type over the alternative?"
- "What tools did you use to reach that conclusion?"

The agent will retrieve and summarize the trace, or you can request the raw trace for detailed inspection.

#### Via MCP (programmatic access)

The Agent MCP Server exposes decision trace tools for external applications:
...
### Use Cases for Decision Traces

#### Auditing AI recommendations

Before accepting an agent's suggestion to restructure a blueprint, review the trace to confirm it considered the right factors:

```bash
dap-bpa decision-trace get dt-20260912-001
```

The trace shows which node types were evaluated, what documentation was consulted, and why the agent chose its recommendation.

#### Learning from agent reasoning

New engineers can study decision traces to understand how experienced practitioners (and the agent) approach blueprint analysis:

1. Ask the agent to analyze a complex blueprint
2. Review the decision trace to see the step-by-step reasoning
3. Note which knowledge base lookups and validation checks informed the conclusions

#### Debugging unexpected recommendations

If the agent suggests something that seems wrong, the trace reveals its reasoning chain so you can identify where it went astray:

- Did it use outdated documentation?
- Did it miss a constraint in the blueprint?
- Did it misinterpret the node type schema?

#### Compliance and governance

For regulated environments, decision traces provide an audit trail of AI-assisted changes:

```bash
# Export all traces from the past week for compliance review
dap-bpa decision-trace export --output weekly-audit.json
```

### Decision Trace Storage

Traces are stored locally under `~/.blueprint-assist/traces/` and are retained for 30 days by default. The storage is append-only and traces are never modified after creation, ensuring audit integrity.

### Best Practices for Decision Traces

1. **Review traces for significant changes**: Before accepting major blueprint restructuring, check the trace
2. **Export traces for team review**: Share traces when discussing agent recommendations in code reviews
3. **Use traces for onboarding**: New team members can learn patterns by studying how the agent reasons
4. **Archive traces for compliance**: Export and store traces if your organization requires AI decision audit trails
...
7. **Review decision traces**: for significant recommendations, check the trace to understand the agent's reasoning

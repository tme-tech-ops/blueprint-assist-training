...
### How the orchestrator, blueprint, and plugins work together

A DAP blueprint declares *what* to deploy, but it does not contain the target-specific logic that actually creates or deletes resources. That logic lives in **plugins**.

When the orchestrator processes a blueprint, it performs the following steps:

1. **Resolves plugin imports.** `blueprint.yaml` imports one or more plugins by name and version range, plus the Dell type library. Only `blueprint.yaml` may import plugins; the split files (`inputs.yaml`, `definitions.yaml`, `capabilities.yaml`) reuse those definitions but do not import plugins themselves.

2. **Loads node types and relationships.** Each plugin exposes a set of node types under the `dell.nodes.<plugin>.<Type>` namespace — for example, `dell.nodes.nativeedge.template.NativeEdgeVM` from the `edge` plugin or `dell.nodes.aws.Instance` from the `aws` plugin. Plugins may also provide relationship types such as `dell.relationships.depends_on`.

3. **Validates the topology.** The orchestrator checks that every `type:` used in `node_templates` exists in an imported plugin and that each property is valid for that type. You can inspect available types and properties with the CLI:
...
4. **Executes lifecycle operations.** During the `install` workflow the orchestrator walks nodes in dependency order and invokes each plugin's `create`, `configure`, `start`, and so on. During `uninstall` it invokes the matching `stop` and `delete` implementations. Plugins translate these lifecycle calls into the actual API calls, CLI commands, or scripts that provision or deprovision resources.

5. **Captures and exposes runtime state.** After a node is created, the plugin populates runtime attributes. The blueprint exposes selected attributes through `capabilities` for users and for other deployments that consume this one.

Plugins are organized by target: cloud (AWS, Azure, GCP, vSphere, OpenStack), containers/orchestration (Kubernetes, Helm, Docker), automation (Ansible, Fabric), IaC (Terraform, Terragrunt), utilities (scripts, REST, general helpers), and others such as libvirt, Redfish, and Serverless. The version range in the `imports` line pins the plugin implementation the orchestrator will load.

```mermaid
flowchart TD
    Blueprint["Blueprint package\nblueprint.yaml\ninputs.yaml\ndefinitions.yaml\ncapabilities.yaml"]
    Plugins["Plugins\nedge / ansible / utilities\nvsphere / aws / kubernetes\n..."]
    Orchestrator["DAP Orchestrator"]
    Deployment["Deployment"]
    Resources["Target resources"]
    Capabilities["Capabilities"]

    Blueprint -->|"imports node types"| Plugins
    Orchestrator -->|"reads & parses"| Blueprint
    Orchestrator -->|"loads implementations"| Plugins
    Orchestrator -->|"creates & manages"| Deployment
    Deployment -->|"provisions / configures"| Resources
    Resources -->|"runtime attributes"| Deployment
    Deployment -->|"exposes values"| Capabilities
```

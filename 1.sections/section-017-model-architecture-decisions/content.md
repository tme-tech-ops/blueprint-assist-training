...
Blueprint Assist is agent- and model-agnostic. It provides the skills and the `bpa` CLI; you bring the agent and the model. The Devin comparison above reflects the common Dell-internal setup, but dap-bpa is not tied to it.

**Agents.** `dap-bpa setup-ide <ide>` installs the dap-bpa skills into any of these targets: `claude-code`, `cursor`, `windsurf` (the Devin IDE), `jetbrains`, `vscode`, and `antigravity`. The skills are plain markdown the agent reads, so any agent that can load them can drive BPA.

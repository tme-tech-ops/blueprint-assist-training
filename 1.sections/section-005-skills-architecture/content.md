...
Skills version with the dap-bpa release: updating the `dap-bpa` binary updates the skill content, and re-running `dap-bpa setup-ide <ide>` pushes the updated skills into your agent. There is no per-skill version to pin or resolve — the release version (e.g. v0.28.2) identifies the whole set. Blueprint versioning is a separate concern, handled with semantic versioning and CHANGELOG.yaml (Section 7).

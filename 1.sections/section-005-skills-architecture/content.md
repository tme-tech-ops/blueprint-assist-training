...
When you run `dap-bpa setup-ide <ide>`, the tool installs a complete skill set into your agent. The following nine skills ship by default (current as of v0.30):

| Skill | Purpose |
| ----- | -------- |
| `blueprint-agent-setup` | Orientation guide for getting started with Blueprint Assist and understanding the agent's capabilities |
| `dap` | Core authoring and monitoring hub - write, review, lint, debug blueprint YAML; manage deployments and secrets |
...
| `mcp-dapo-setup` | Configure and troubleshoot connections to the Blueprint Assist MCP server endpoint |
| `my-dap-patterns` | Learn, review, validate, and apply your organization's blueprint conventions (naming, IP ranges, VLANs, policy bindings, module structure) |
| `script-plugin` | Write and debug Python scripts for DAP blueprints using the `dell` context API |
| `service-composition` | Compose services using ServiceComponent sub-deployments, SharedResource, and blueprint chaining |
| `verify-skills` | Verify the integrity and signatures of installed skills |
| `visualize-blueprint` | Generate interactive HTML topology diagrams showing nodes, relationships, inputs, and plugins |
...
To see exactly which skills your release will install, use:

```bash
dap-bpa setup-ide list-skills
```

This command is the authoritative, self-serve way to check what's available - more durable than any hardcoded list, since it reflects your actual release version.

## Rules Files

Alongside skills, `setup-ide` installs three agent rules files that define authoring standards and compliance checks:

| Rules File | Purpose |
| ---------- | -------- |
| `blueprint-rules.md` | Core blueprint authoring standards - naming conventions, structure, and best practices |
| `dap-rules.md` | DAP-specific compliance rules for blueprint validation and linting |
| `dap-monitor-check.md` | Proactive checks run at the start of monitoring workflows to validate deployment readiness |

These rules are installed separately from the skills directory into your agent's rules configuration. They work alongside the skills to enforce consistency and catch issues early. The linter (`dap-bpa blueprint lint`) enforces these rules during authoring.

## Upgrading and Deprecated Skills

When you upgrade `dap-bpa` and re-run `setup-ide`, the tool deliberately removes approximately 50 old skill names from your skill directories:
...
**This is expected behavior.** Stale skills carry outdated instructions that can mislead the agent. Removing them ensures the agent only consults current, release-aligned guidance. The new skill names (e.g., `dap`, `script-plugin`, `deployment-update`) replace the old ones with updated content.

If you have custom skills in your skill directories, they are preserved during upgrade.
...
The "engine" of the architecture is a loop between the agent's LLM and the `dap-bpa` CLI:

- The LLM reads the skills to understand how to work
- It calls the `dap-bpa` CLI for knowledge lookup, linting, and validation
...
- **Refinement loop**: the LLM calls the `dap-bpa` CLI for knowledge, linting, and validation, and refines the blueprint until the checks pass
...
Skills version with the dap-bpa release: updating the `dap-bpa` binary updates the skill content, and re-running `dap-bpa setup-ide <ide>` pushes the updated skills into your agent. There is no per-skill version to pin or resolve - the release version (e.g. v0.29.3) identifies the whole set. Blueprint versioning is a separate concern, handled with semantic versioning and CHANGELOG.yaml (Section 7).

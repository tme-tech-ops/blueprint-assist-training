...
| `dap-bpa setup-ide <ide>` | Install dap-bpa skills into a supported IDE. Supported IDEs: windsurf, claude-code, cursor, jetbrains, vscode, antigravity, cline (`cline` added v0.28.1+; the `antigravity` target also covers Codex CLI, Gemini CLI, and Goose) |
...
| `dap-bpa upgrade --file <zip>` | Replace running binary from an explicit distribution zip |
...
| `dap-bpa upgrade --rollback` | Restore the previous binary from its timestamped `.bak` backup (v0.28.0+) |
...
| `dap-bpa blueprint visualize --file <path> [--output <path>]` | Generate a self-contained interactive HTML visualization of the blueprint (see [Section 21](../section-021-blueprint-visualizer/content.md)). As of v0.28.2 this produces a React-based diagram (install-flow, platform detection, plugins, and a Risk Analysis panel) with React inlined and no CDN requests; `.riskignore` next to the blueprint root suppresses risk findings. The `--file` path must be within the current working directory |
...
| Command | Purpose |
| --------- | --------- |
| `dap-bpa orchestrator blueprints upload --file <path> --id <id> --revision <ver>` | Upload a blueprint archive; prints the DAP console inventory URL and a local checksum on success for integrity verification (v0.28.0+) |
...
| `dap-bpa monitor --daemon-stop` | Stop the background daemon (no orchestrator auth required, v0.28.0+) |
...
- **CLI version**: this reference covers dap-bpa v0.28.2

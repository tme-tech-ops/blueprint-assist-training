...
Research the plugins you'll need before writing node templates. Plugins determine what a blueprint can actually deploy: they provide node types, required properties, lifecycle implementations, runtime attributes, and example patterns for each target platform.

Start by mapping the requirement to a plugin family:

| Requirement | Plugin family to inspect |
| ----------- | ------------------------ |
| Virtual machines on vSphere or Dell infrastructure | `vsphere`, `edge` |
| Public cloud resources | `aws`, `azure`, `gcp`, `openstack`, `vcloud` |
| Kubernetes applications or Helm charts | `kubernetes`, `helm` |
| Configuration management or scripts | `ansible`, `fabric`, `utilities` |
| Infrastructure-as-Code modules | `terraform`, `terragrunt` |

Then inspect the exact node types and schemas:
...
Use the plugin documentation to decide:

- Which `plugin:<name>?version=...` line belongs in `blueprint.yaml`
- Which `type:` values are valid under `node_templates`
- Which properties must be supplied from inputs, secrets, anchors, or attributes
- Which runtime attributes can be read with `get_attribute`
- Which existing examples demonstrate the plugin in a complete topology

Do not guess node type names or property names. If a node type is not returned by `knowledge plugins list` or `knowledge plugins get`, treat it as unavailable for that plugin.
...
Ask the 'dap' Skill to create a Blueprint from your Blueprint definition.

The skill will create your `blueprint.yaml` following TOSCA standards:

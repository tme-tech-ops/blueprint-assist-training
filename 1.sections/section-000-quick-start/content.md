...
1. **`bpa` CLI** — runs locally on your machine. Lints blueprints, queries plugin docs, talks to the DAP orchestrator, and runs automated lifecycle tests.
...
You use both together: chat with your agent to design and author blueprints, use `bpa` commands to validate and deploy them.
...
If `bpa` is not found, open a new terminal so the PATH update takes effect.

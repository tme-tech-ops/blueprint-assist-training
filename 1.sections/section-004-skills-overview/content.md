...
Blueprint Assist ships nine skills by default (current as of v0.31). They are installed into your agent with `dap-bpa setup-ide <ide>` (Section 2) and activate automatically from natural-language prompts - there is no separate skill invocation step.

> **v0.31.0 breaking change**: The `windsurf` IDE target is replaced by `devin`; `codex` is restored as a named target. Existing Windsurf installs are still synced by `upgrade` and scanned by `verify-skills`.
...
| `verify-skills` | Verifying the integrity and signatures of installed skills; classifies each as dell-provided, customer-modified, unsigned, or invalid-signature (v0.31.0+: audit logging to `~/.blueprint-assist/skill-verify.log`, `--report` compliance output) |

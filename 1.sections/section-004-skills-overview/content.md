...
Blueprint Assist ships five skills as of v0.27.0. They are installed into your agent with `dap-bpa setup-ide <ide>` (Section 2) and activate automatically from natural-language prompts — there is no separate skill invocation step.
...
| `dell-sdd` | Spec Dev Design workflows (v0.27.0+) |
...
As of v0.24.0 and continuing through v0.27.0, all dap-bpa skills use the `dap-*` prefix (previously `blueprint-assist-*`). Examples:
...
- **Skills** (the five above) teach the agent workflows: authoring, scripting, deployment updates, service composition, and spec-driven development. Monitoring and lifecycle-testing guidance is part of the core `dap` skill.

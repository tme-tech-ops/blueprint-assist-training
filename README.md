...
- **Docker Distribution** (v0.27.0+): BPA CLI available as a Docker image for containerized and CI/CD workflows; Docker image builds/publishes were restored in v0.29.3
- **Self-Updating CLI** (v0.28.0+): `dap-bpa upgrade` auto-discovers a downloaded release zip and installs it, `dap-bpa upgrade --check` gates CI on new versions (exit `10` = update available), and `dap-bpa upgrade --rollback` atomically restores the previous binary from its `.bak` backup; v0.29.3 sweeps any stale `.bpa-upgrade-<ts>`, `.bpa-daemon-upgrade-<ts>`, and `.bpa-rollback-<ts>` staging files at startup so interrupted upgrades/rollbacks don't leak files
...
- **Wiki Knowledge Base Browser**: `dap-bpa wiki` lets you browse the local knowledge base from the terminal with `toc`, `search`, `read`, `status`, `lint`, and `synthesize` commands
- **Customer Pattern Learning**: the `my-dap-patterns` skill and `dap-bpa knowledge patterns` commands learn, review, validate, and apply your own blueprint conventions (naming, IP ranges, VLANs, policy bindings, module structure, variable names)
- **In-IDE Visualizer Preview**: `dap-bpa blueprint visualize --serve` launches a local HTTP server for in-IDE preview (Windsurf/VS Code Simple Browser) and adds `--diff`, `--export`, and `--execution-status-url` options
- **Question Engine & Prompt Coaching** (v0.30.1+): `dap-bpa questions ask --intent` and `dap-bpa questions update --answers` for interactive prompt refinement and coaching workflows
- **Decision Trace** (v0.30.1+): Decision-record schema, trace composer with persistence, IDE trace display, and MCP access to decision traces for full reasoning transparency
- **GCP Platform Support** (v0.30.1+): GCP natural-language patterns, example blueprints, and schema/credential validation in the linter for Google Cloud deployments
- **Enhanced Pattern Learning** (v0.30.1+): Pattern MCP tools, pattern-learning agent, pattern feedback command (`dap-bpa knowledge patterns feedback`), and structured attribution metadata
- **Analytics Dashboard** (v0.30.1+): Full dashboard UI with export API and Dell Identity authentication for deployment analytics and insights
- **Wiki Synthesis** (v0.30.1+): `dap-bpa wiki` command with maintainer synthesis and scheduled synthesis workflows for knowledge base maintenance
- **macOS Binary Signing** (v0.30.1+): Binaries now signed with Dell Developer ID certificate for enhanced security and distribution
- **Local Plugin Schema Validation** (v0.30.1+): Validate plugin schemas locally without orchestrator connection for faster development iteration
- **GCP Agent Generation** (v0.31.0+): Complete GCP platform support with natural-language resource extraction (`gcp_extract_resources`), deterministic blueprint YAML generation, bounded lint-and-repair loop (max 5 cycles), multi-turn refinement (`gcp_refine_blueprint`), secret masking for PEM/service-account JSON, and offline fallback with `degraded_accuracy: true`
- **Skill Signature Verification** (v0.31.0+): `dap-bpa verify-skills` scans installed skills and classifies each as dell-provided, customer-modified, unsigned, or invalid-signature (exit 1 on invalid); `setup-ide` surfaces verification status during install; audit logging to `~/.blueprint-assist/skill-verify.log`
- **MCP Token Minting** (v0.31.0+): `dap-bpa mint-token` mints MCP gateway tokens with correct recipe (scope=internal:exchangeable + --audience), with `--verify` flag for gateway validation and `--export`/`--json`/`--out`/`--copy` output modes
- **Decision Trace CLI** (v0.31.0+): `dap-bpa trace record/compose/show` commands for file-backed decision recording; decision trace now wired end-to-end across REST, MCP, and CLI/IDE paths
- **Origin Indicator Stamping** (v0.31.0+): `dap-bpa blueprint stamp --file <path> [--check]` stamps blueprints at generation time (not just upload); deterministic stamp-if-absent behavior
- **Directory Upload Support** (v0.31.0+): `orchestrator blueprints upload/install --file` now accepts a directory (auto-tars with YAML entrypoint detection)
- **Post-Generation Follow-Ups** (v0.31.0+): AI-generated follow-up suggestions across default-applied, assumption, and low-confidence categories surfaced in REST `/invoke`, `ask_bpa_agent`, and IDE extension
- **Analytics Dashboard Completion** (v0.31.0+): Full dashboard with OIDC sign-in (Dell Identity PKCE flow), frontend RBAC, rate limiting, real KPIs/cohort/retention charts, and advanced visualizations; analytics enabled for staging/production MCP servers
- **Docker Sandbox Images** (v0.31.0+): `dap-bpa-sandbox-claude` / `dap-bpa-sandbox-codex` containers for testing at any release tag or commit without touching host install
- **Skill Benchmark Program** (v0.31.0+): Deterministic + LLM-judged benchmark cases for skills with `bpa-benchmark-data/history.json` result store and weekly skill-analysis-tool that opens improvement issues
...
```text
blueprint-assist-training/
+-- 1.sections/              # Training course sections (0-29)
...
¦   +-- section-023-customer-patterns/
¦   +-- section-024-gcp-agent-generation/
¦   +-- section-025-docker-sandbox-images/
¦   +-- section-026-mcp-token-management/
¦   +-- section-027-question-engine/
¦   +-- section-028-skill-benchmark-program/
¦   +-- section-029-analytics-dashboard/
...
¦   +-- TRAINING-AGENDA.md       # Two-day instructor-led training agenda
...
- [Dell Software License Agreements (EULA)](https://www.dell.com/en-us/lp/legal/art-software-license-agreements)
...
This training is organized into 30 sections, starting with a Quick Start for new users:
...
9. [**Blueprint Reasoning**](1.sections/section-009-blueprint-reasoning/content.md) - AI-powered analysis, explanation, understanding of blueprints, and decision trace transparency (no DAP connection required)
...
15. [**Dell Automation Studio Offer Details**](1.sections/section-015-studio-offer-details/content.md) - Offer and packaging details for the paid subscription tier (licensing mechanics in Section 22)
...
19. [**Blueprint Assist MCP Server**](1.sections/section-019-mcp-server/content.md) - Model Context Protocol (MCP) server for programmatic blueprint operations, Question Engine, Decision Trace API, and Analytics Dashboard
...
22. [**Licensing Model**](1.sections/section-022-licensing-model/content.md) - Commercial reference for sellers/account teams: MRUs, deployment credits, differentiators, and licensing scenarios
23. [**Customer Pattern Learning**](1.sections/section-023-customer-patterns/content.md) - Extract, curate, and enforce organizational blueprint conventions (naming, IP ranges, VLANs, policies) with the pattern learning engine
24. [**GCP Agent Generation**](1.sections/section-024-gcp-agent-generation/content.md) - Natural-language Google Cloud resource extraction, deterministic blueprint YAML generation, and multi-turn refinement for GCP deployments (v0.31.0+)
25. [**Docker Sandbox Images & Testing**](1.sections/section-025-docker-sandbox-images/content.md) - Containerized testing at any release version without host modification; CI/CD integration examples for GitHub Actions, GitLab, and Jenkins (v0.31.0+)
26. [**MCP Token Management**](1.sections/section-026-mcp-token-management/content.md) - Secure token minting, validation, and API integration; OAuth workflows and REST API examples for orchestrator authentication (v0.31.0+)
27. [**Question Engine & Prompt Coaching**](1.sections/section-027-question-engine/content.md) - Interactive multi-turn Q&A for guided blueprint authoring; assumption tracking and iterative refinement for complex infrastructure (v0.30.1+)
28. [**Skill Benchmark Program**](1.sections/section-028-skill-benchmark-program/content.md) - Deterministic skill testing with LLM-judged evaluation; result tracking, trend analysis, and automated quality improvement workflows (v0.31.0+)
29. [**Analytics Dashboard Deep Dive**](1.sections/section-029-analytics-dashboard/content.md) - Deployment insights, KPI dashboards, cost analysis, team-based metrics, and governance for infrastructure automation (v0.30.1+)
...
Blueprint Assist and the Dell Automation Platform software are governed by the applicable Dell End User License Agreement (EULA). See [Dell Software License Agreements](https://www.dell.com/en-us/lp/legal/art-software-license-agreements).

For ordering and licensing specifics, see the [Dell Automation Platform Ordering and Licensing Guide](https://www.delltechnologies.com/asset/en-us/solutions/infrastructure-solutions/legal-pricing/dell-automation-platform-ordering-licensing-guide.pdf) and [Section 022: Licensing Model](1.sections/section-022-licensing-model/content.md). For additional licensing information, refer to the main Blueprint Assist project.

This training repository's content is provided "as is"; see the [Disclaimer](DISCLAIMER.md).
